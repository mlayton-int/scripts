...
<#
.SYNOPSIS
    Deletes all but the newest N volume shadow copies (system restore points).

.DESCRIPTION
    Opt-in disk reclaim task, split out of Invoke-DiskCleanup.ps1. Enumerates
    Win32_ShadowCopy, keeps the -KeepRestorePoints newest, and removes the rest.

    WHAT THIS TRADES AWAY
      System Restore rollback points, and any Previous Versions a user might recover a
      file from. Reclaimed space is measured by free-space delta because shadow copies
      do not have a straightforward per-item size.
...
.PARAMETER KeepRestorePoints
    Number of newest shadow copies to keep. Default 1.
...
try {
    ...
    Write-Log "KeepRestorePoints: $KeepRestorePoints"
    ...
    Invoke-Task -Name 'RestorePoints' -Action {
