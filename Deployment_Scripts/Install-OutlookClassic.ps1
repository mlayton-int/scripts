...
# Microsoft 365 channel. Common values:
#   Current              = Monthly Enterprise (latest features)
#   MonthlyEnterprise    = Monthly Enterprise Channel (one month behind, more stable)
#   SemiAnnual           = Semi-Annual Enterprise Channel (most stable, patched twice/year)
$UpdateChannel = "MonthlyEnterprise"

# Target Office platform. Use "64" for all modern workstations.
$Architecture = "64"

# Accepts the Microsoft Office license agreement automatically.
# Set to $false to review before deploying.
$AcceptEula = $true
...
Write-Log "UpdateChannel : $UpdateChannel"
...
# -- Helper: write ODT configuration XML --------------------------------------
function New-OdtConfigXml {
    ...
    $eulaValue = if ($Eula) { "TRUE" } else { "FALSE" }
...
<Configuration>
  <Add OfficeClientEdition="$Arch" Channel="$Channel">
    <Product ID="O365ProPlusRetail">
      <Language ID="MatchOS" />
      <!--
        ExcludeApp blocks everything except Outlook Classic.
        Remove or comment out any line to include that app.
      -->
  ...
  <Display Level="None" AcceptEULA="$eulaValue" />
  <Logging Level="Standard" Path="C:\INS-Temp\Logs" />
</Configuration>
"@

    Set-Content -Path $OutputPath -Value $xml -Encoding UTF8
    Write-Log "ODT config XML written to: $OutputPath"
}
...
try {
    ...
    if (-not (Test-Path -LiteralPath $OdtSetupPath -PathType Leaf)) {
        ...
        $odtDownloadUrl = "https://download.microsoft.com/download/2/7/A/27AF1BE6-DD20-4CB4-B154-EBAB8A7D4A7E/officedeploymenttool_17830-20162.exe"
    ...
    # 3. Write ODT configuration XML -------------------------------------------
    $configXmlPath = Join-Path (Split-Path $OdtSetupPath -Parent) "outlook-config.xml"
    New-OdtConfigXml -OutputPath $configXmlPath `
